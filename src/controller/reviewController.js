...
const createReview = async function (req,res){
    ...
    if (!isValidRequestBody(data)) res.status(400).send({status:false, message: "data is required"})

    if (!isValidObjectId(bookId)) res.status(400).send({status:false, message: "bookId is required"})
